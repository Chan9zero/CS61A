...
def catch_up(score, opponent_score):
    ...
    >>> catch_up(9, 4)
    5
    >>> strategy(17, 18)   Chan9zero 注：这里应该为catch_up(17, 18)
    6
    """
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    i, average, max, max_num = 1, 0, 0, 0
    average_dice = make_averaged(roll_dice, samples_count)
    for i in range(1,11):
        average = average_dice(i, dice)
        if average > max:
            max, max_num = average, i
    return max_num
...
def run_experiments():
    ...
    print('always_roll(5) win rate:', average_win_rate(always_roll(6)))
